...
import { getResumeSuggestions, rewriteAllBullets } from "./lib/ai-secure";
  ...
  for (const d of delimiters) {
    ...
    if (idx !== -1) {
      return {
        ...
        right: text.slice(idx + d.length).trim(),
      };
    }
  }
...
export default function ResumeEditor() {
    ...
    setResume((r) => ({
      ...
      sections: [
        ...r.sections,
        {
          id,
          title: "New Section",
          items: [{ heading: "", subheading: "", bullets: ["New bullet"] }],
        },
      ],
    }));
  }
  ...
  function addItem(sectionId) {
    updateSection(sectionId, (s) => ({
      ...
      items: [
        ...s.items,
        { heading: "", subheading: "", bullets: ["New bullet"] },
      ],
    }));
  }
  ...
  function addBullet(sectionId, itemIdx) {
    updateSection(sectionId, (s) => {
      const items = s.items.map((it, i) =>
        i === itemIdx ? { ...it, bullets: [...it.bullets, "New bullet"] } : it
      );
      return { ...s, items };
    });
  }
  ...
  function removeBullet(sectionId, itemIdx, bIdx) {
    updateSection(sectionId, (s) => {
      const items = s.items.map((it, i) =>
        i === itemIdx
          ? { ...it, bullets: it.bullets.filter((_, b) => b !== bIdx) }
          : it
      );
      return { ...s, items };
    });
  ...
  function applyRewriteSuggestions() {
    ...
    setResume(rewriteSuggestions);
    setRewriteSuggestions(null);
  }

  function dismissRewriteSuggestions() {
    setRewriteSuggestions(null);
  }

  function hasChangedBullets(sectionId, itemIndex) {
    if (!rewriteSuggestions) return false;
    const originalSection = resume.sections.find(s => s.id === sectionId);
    const rewrittenSection = rewriteSuggestions.sections.find(s => s.id === sectionId);
    if (!originalSection || !rewrittenSection) return false;
    
    const originalItem = originalSection.items[itemIndex];
    const rewrittenItem = rewrittenSection.items[itemIndex];
    if (!originalItem || !rewrittenItem) return false;
    
    return JSON.stringify(originalItem.bullets) !== JSON.stringify(rewrittenItem.bullets);
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-500 via-gray-500 to-gray-500 relative overflow-hidden">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setResume({ ...resume, contact: e.target.value })
                    }
                          ...
                          <div className="flex items-start gap-3 mb-3">
                            <div className="flex-1 space-y-3">
                              <Input
                                ...
                                onChange={(e) =>
                                  updateField(
                                    ["sections", sIdx, "items", idx, "heading"],
                                    e.target.value
                                  )
                                }
                                className="bg-gray-900/50 border-gray-600 text-gray-100 placeholder:text-gray-400 focus:border-blue-500 focus:ring-blue-500/20"
                              ...
                              >
                                <div className="flex flex-col gap-1 mt-1">
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      moveBullet(section.id, idx, bi, bi - 1)
                                    }
                                    disabled={bi === 0}
                                ...
                                <div className="flex flex-col gap-1">
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      changeIndent(section.id, idx, bi, 1)
                                    }
                                    className="h-6 w-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity text-gray-300 hover:bg-gray-700/50 hover:text-gray-100"
                                    title="Indent right"
                                  >
                                  ...
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      removeBullet(section.id, idx, bi)
                                    }
                ...
                <Card className="border-0 shadow-2xl bg-gradient-to-br from-blue-900/20 to-gray-900/20 backdrop-blur-md border border-blue-700/30">
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                      <div className="sm:col-span-2 space-y-2"></div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        onClick={async () => {
                          setAiLoading(true);
                          const res = await getResumeSuggestions(resume);
                      ...
                      <Button
                        variant="outline"
                        disabled={aiLoading || rewriteSuggestions}
                        onClick={async () => {
                          setAiLoading(true);
                          const { resume: rewritten } = await rewriteAllBullets(resume);
                          setRewriteSuggestions(rewritten);
                          setAiLoading(false);
                        }}
                        className="bg-gray-800/50 border-blue-600/50 text-gray-200 hover:bg-blue-900/30"
                      >
                        {rewriteSuggestions ? "✓ Generated" : "Rewrite Bullets"}
                      </Button>
                    </div>

                    {rewriteSuggestions && (
                      <div className="space-y-3 p-4 bg-purple-900/20 rounded-lg border border-purple-700/30 backdrop-blur-sm">
                        <div className="flex items-center justify-between">
                          <div className="text-sm font-semibold text-purple-300">
                            Rewritten Bullets Ready
                          </div>
                          <button
                            onClick={dismissRewriteSuggestions}
                        ...
                        <p className="text-sm text-gray-300">
                          AI has rewritten your bullet points to be more impactful. Review the changes below.
                        </p>
                        <div className="flex gap-2">
                          <Button
                            size="sm"
                            onClick={applyRewriteSuggestions}
                    ...
                    {(aiSuggestions.summary ||
                      (aiSuggestions.suggestions?.length || 0) > 0) && (
                ...
                <CardHeader className="pb-4">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-lg flex items-center gap-2 text-gray-100">
                      ...
                      {rewriteSuggestions ? "Preview: Rewritten Resume" : "Resume Preview"}
                    </CardTitle>
                    <Badge
                      variant="secondary"
                      className="text-xs bg-blue-900/50 text-blue-200 border border-blue-700/50"
                    >
                      {(rewriteSuggestions || resume).sections.reduce(
                        (total, section) =>
                          total +
                  ...
                  {rewriteSuggestions && (
                    <div className="px-4 py-2 bg-purple-900/20 border-b border-purple-700/30">
                      <p className="text-xs text-purple-300">
                         Previewing rewritten version - bullets have been improved by AI
                      </p>
                    </div>
                  )}
                </CardHeader>
                  ...
                  <div className="border border-gray-600/50 rounded-lg overflow-hidden">
                    <div id="print-area" className="bg-white p-8">
                      <div className="border-b pb-3 mb-4 text-center">
                        <h1
                          className="text-xl font-bold mb-1 text-[#1f4e79]"
                          style={{ fontSize: 21 }}
                        >
                          {(rewriteSuggestions || resume).name}
                        </h1>
                        <p className="text-xs text-gray-700">
                          {(rewriteSuggestions || resume).contact}
                        </p>
                      </div>

                      {/* Sections */}
                      <div className="space-y-3">
                        {(rewriteSuggestions || resume).sections.map((s) => (
                                ...
                                <div key={i} className="mb-2">
                                  ...
                                  <ul className="space-y-0.5">
                                    {it.bullets.map((b, bi) => {
                                      ...
                                      // Check if this bullet changed
                                      const isChanged = rewriteSuggestions && 
                                        resume.sections.find(sec => sec.id === s.id)?.items[i]?.bullets[bi] !== b;
                                      ...
                                      return (
                                        <li
                                          ...
                                          style={{
                                            ...
                                            marginBottom: "1px",
                                            backgroundColor: isChanged ? '#fef3c7' : 'transparent',
                                            padding: isChanged ? '2px 4px' : '0',
                                            borderRadius: isChanged ? '2px' : '0',
                                          }}
                                        >
                                          <span
      ...
      <style jsx>{`
        ...
        .glow-emerald::after {
          box-shadow: 0 0 32px 4px rgba(16, 185, 129, 0.35);
        }
        .glow-blue::after {
          box-shadow: 0 0 32px 4px rgba(59, 130, 246, 0.35);
        }
        .glow-orange::after {
          box-shadow: 0 0 32px 4px rgba(249, 115, 22, 0.35);
        }
        @keyframes glowPulse {
          0%,
          100% {
